...
## 🔗 Linear Collection Mechanics: LinkedList Implementation

### 📋 Key Conceptual Takeaways
* **Satisfying the Interface Contract:** Just like our arrays and strings, the `LinkedList` successfully plugs into our `Sorter` class by exposing standardized `length`, `compare()`, and `swap()` implementations [source: 3].
* **The Traversal Tax:** Unlike index-accessible arrays, a Linked List cannot immediately read data at a target index. The `at()` method must linearly step through individual node pointers (`node = node.next`), making lookups an $O(N)$ operation instead of an $O(1)$ constant time lookup.
* **The Pointer Re-link "Cheat":** True node-pointer manipulation in a sorting algorithm requires updating multiple `.next` references in memory. To keep the code highly concise, our `swap()` method swaps the inner numeric primitive `.data` values directly, achieving the sort without breaking or rebuilding the reference chains.

### 💻 LinkedList Implementation with Documentation
```typescript
/**
 * ============================================================================
 * CLASS: Node
